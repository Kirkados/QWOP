...
#    #xfl,yfl = next_state[29:31]
    ...
    x1r = x - env.SEGMENT_GAMMA_LENGTH[0] * np.sin(theta) + env.SEGMENT_GAMMA_LENGTH[1] * np.sin(theta1r)
    y1r = y + env.SEGMENT_GAMMA_LENGTH[0] * np.cos(theta) - env.SEGMENT_GAMMA_LENGTH[1] * np.cos(theta1r)
    x2r = x1r + (env.SEGMENT_LENGTH[1] - env.SEGMENT_GAMMA_LENGTH[1]) * np.sin(theta1r) + env.SEGMENT_GAMMA_LENGTH[2] * np.sin(theta2r)
    y2r = y1r - (env.SEGMENT_LENGTH[1] - env.SEGMENT_GAMMA_LENGTH[1]) * np.cos(theta1r) - env.SEGMENT_GAMMA_LENGTH[2] * np.cos(theta2r)
    x3r = x + (env.SEGMENT_LENGTH[0] - env.SEGMENT_GAMMA_LENGTH[0]) * np.sin(theta) + env.SEGMENT_GAMMA_LENGTH[3] * np.sin(theta3r)
    y3r = y - (env.SEGMENT_LENGTH[0] - env.SEGMENT_GAMMA_LENGTH[0]) * np.cos(theta) - env.SEGMENT_GAMMA_LENGTH[3] * np.cos(theta3r)
    x4r = x3r + (env.SEGMENT_LENGTH[3] - env.SEGMENT_GAMMA_LENGTH[3]) * np.sin(theta3r) + env.SEGMENT_GAMMA_LENGTH[4] * np.sin(theta4r)
    y4r = y3r - (env.SEGMENT_LENGTH[3] - env.SEGMENT_GAMMA_LENGTH[3]) * np.cos(theta3r) - env.SEGMENT_GAMMA_LENGTH[4] * np.cos(theta4r)

    x1l = x - env.SEGMENT_GAMMA_LENGTH[0] * np.sin(theta) + env.SEGMENT_GAMMA_LENGTH[1] * np.sin(theta1l)
    y1l = y + env.SEGMENT_GAMMA_LENGTH[0] * np.cos(theta) - env.SEGMENT_GAMMA_LENGTH[1] * np.cos(theta1l)
    x2l = x1l + (env.SEGMENT_LENGTH[1] - env.SEGMENT_GAMMA_LENGTH[1]) * np.sin(theta1l) + env.SEGMENT_GAMMA_LENGTH[2] * np.sin(theta2l)
    y2l = y1l - (env.SEGMENT_LENGTH[1] - env.SEGMENT_GAMMA_LENGTH[1]) * np.cos(theta1l) - env.SEGMENT_GAMMA_LENGTH[2] * np.cos(theta2l)
    x3l = x + (env.SEGMENT_LENGTH[0] - env.SEGMENT_GAMMA_LENGTH[0]) * np.sin(theta) + env.SEGMENT_GAMMA_LENGTH[3] * np.sin(theta3l)
    y3l = y - (env.SEGMENT_LENGTH[0] - env.SEGMENT_GAMMA_LENGTH[0]) * np.cos(theta) - env.SEGMENT_GAMMA_LENGTH[3] * np.cos(theta3l)
    x4l = x3l + (env.SEGMENT_LENGTH[3] - env.SEGMENT_GAMMA_LENGTH[3]) * np.sin(theta3l) + env.SEGMENT_GAMMA_LENGTH[4] * np.sin(theta4l)
    y4l = y3l - (env.SEGMENT_LENGTH[3] - env.SEGMENT_GAMMA_LENGTH[3]) * np.cos(theta3l) - env.SEGMENT_GAMMA_LENGTH[4] * np.cos(theta4l)
    ...
    while running:
        ...
        if begin or not play_game:
            ...
            # KH added to prevent game-over text being displayed on July 29, 2019.
            # Made a similar change in environment_qwop_full_11.py on line 445
            GAME_OVER_ON_FALL = False

            #check if node out-of-bounds. If so, call it quits
            if np.any(segment_points[:,0,1]>height) and GAME_OVER_ON_FALL:
